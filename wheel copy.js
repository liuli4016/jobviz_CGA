...
var translateX = 0.88*diameter*initialScale;
var translateY = 0.66*diameter*initialScale;
...
//$("verticalLegend").css({bottom: 20px, left: 2px, position:'fixed'});

  d3.select("svg")
  .append("g")
  .attr("transform", "translate(0, 550)scale(0.75, 0.75)")
  .attr("class", "legend")
  .call(verticalLegend);
...
var subgroupsign = 0;
var currentDbcText = "Jobs of 2016";
...
{	
    ...
    for (var i = 0; i < words.length; i++)
	  ...
	  for (var i = 0; i < words.length; i++)
	  {
	   ...
	   {
	      displaytag_num = Math.min(40, frequency_list.length);
	      wordscalefactor = cloudWidth/(5*max_frequency);
	   }
	      
	   else if (frequency_list.length > 150 && frequency_list.length <= 500)
	   {
	   	  displaytag_num = 55;
	   	  wordscalefactor = cloudWidth/(7*max_frequency);
	   }
	   ...
	   {
	   	  displaytag_num = 100;
	   	  wordscalefactor = cloudWidth/(10*max_frequency);
	   }
...
//console.log(displaytag_num);
     ...
     var wordsize = 0;   
        ...
        { 
        	...
        	*/
   /*
           if (d[0] == "communicate" || d[0] == "nonprofit")
        	 ...
        	 if (d[0] == "science" || d[0] == "develop" || d[0] == "management")  
        	    wordsize = wordsize*0.2;
   */
   
 /*  
 if (d[0] == "software" || d[0] == "develop" || d[0] == "management" || d[0] == "technology" || d[0] == "travel"|| d[0] == "academic" || d[0] == "communication")  
        	    wordsize = wordsize*0.5;
        	    
    if (d[0] == "artist" || d[0] == "art" || d[0] == "athletic" || d[0] == "teaching")  
        	    wordsize = wordsize*2;
 */  
  ...
  if (d[0] == "environment")
     wordsize = wordsize*0.4;
        	 ...
        	 else if (wordsize < cloudWidth/135)
        	    wordsize = cloudWidth/135;
        ...
        //.padding(3)
        .padding(function (d) { return Math.max(6, d.size/15); })
        .spiral('archimedean')
        //.spiral('rectangular')
        .font("Impact")
 ...
 */	
    ...
    .classed("link--source", function(l) { return l.target.target = false; });	
	  ...
	  svg.selectAll(".jobgroupnode").remove();	 
    addCareerSectorLabel();
    svg.selectAll(".jobgroupnode").filter(function(e) { return e.data.index == groupIDByIndex.get(d.name); }).classed("connectjobgroupnode", true).style("fill", function(d) { return colorByindex.get(d.data.index); });
...
function drawmajornode()
{
       ...
       node = node.data(nodes.filter(function(n) { return (!n.children)&&(n.id.substring(0, n.id.indexOf("@"))=="maj"); }))
              ...
              {	
              	   if (connectiondirection == "Major->Job" && selectedmajor.length > 0)
              	   {
              	   ...
              	   {             	   	  
              	      ...
              	      /*
              	      else if ( connectedmajornodes2.length > 0 )
              	   	  {             	   	  
              	         node.filter( function(e) 
              	         { 
              	      	     var alreadyselected = 0;
              	      	     for (var i=0; i<connectedmajornodes2.length; i++)
              	      	     {
              	      	  	     if (e.key == connectedmajornodes2[i])
              	      	  	        alreadyselected = 1;
              	      	     }
...
var chartcontainer = d3.select("#summaryview").append("svg").attr("width", 235).attr("height", 240);  
...
function drawBarchart(displayarray, displaylabel, displaycolor)
{
    ...
    var totalWidth = 235;
    var totalHeight = 240;
    var horizontal_shift = 35;
    var vertical_shift = 60;
...
{ 
    ...
    document.getElementById("majortab").style.backgroundColor = "white";    	 	
	  ...
	  chartcontainer = d3.select("#summaryview").append("svg").attr("width", 235).attr("height", 290); 	  	  
	  
    var w = 184, h = 184, r = Math.min(w, h) / 2,
    ...
    var donutarcs = chartcontainer.selectAll("g.arc")
                    ...
                    .enter().append("svg:g")
                    .attr("class", "arc")
                    .attr("transform", "translate(" + (r + 28) + "," + (r + 36) + ")");
